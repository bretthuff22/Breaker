...
struct SPoint
{
	...
	SPoint(float _x, float _y, DWORD _rgb) :
		position(_x, _y, 1.0f, 1.0f),
		rgb(_rgb)
	{ }
};
...
const int c_MaxNumPoints = 800 * 600;
static SPoint s_PointList[c_MaxNumPoints];
static int s_PointIndex = 0;

const int c_MaxNumLines = 50000;
...
void AddScreenPoint(float x, float y, DWORD rgb)
{
	// Check if we have initialized
	if (!s_Initialized)
	{
	...
	if (s_PointIndex + 1 >= c_MaxNumPoints)
	{
		return;
	}
	...
	float xx = (int)x + 0.5f;
	float yy = (int)y + 0.5f;
	s_PointList[s_PointIndex++] = SPoint(xx, yy, rgb);
}
...
void Render(void)
{
	...
	if (s_PointIndex > 0)
	{
		CDXGraphics::Get()->D3DDevice()->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
		CDXGraphics::Get()->D3DDevice()->DrawPrimitiveUP(D3DPT_POINTLIST, s_PointIndex, s_PointList, sizeof(SPoint));
	}
